#!/usr/bin/env python3
"""Build a single .docx from the Ortho1 markdown summary files."""
...
OUTPUT = BASE / "Ortho1_Samenvatting.docx"

FILES = [
    "00_Leerarchitectuur.md",
    "01_Kernkader.md",
    "02_Historisch_Overzicht.md",
    "03_Vergelijkingsmatrix.md",
    "04_Verleden_is_Heden.md",
    "05_Zelftoets.md",
]
...
def strip_details_tags(text):
    """Convert <details><summary>X</summary> blocks into visible text."""
...
def process_markdown(doc, md_text, base_path):
    ...
    md_text = strip_details_tags(md_text)
    lines = md_text.split('\n')
    i = 0
...
def main():
    ...
    doc.add_paragraph("")
    doc.add_paragraph("")
    title = doc.add_heading("Orthopedagogie en Maatschappij", level=0)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    sub = doc.add_paragraph()
    sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_styled_run(sub, "Hoofdstuk 1 — Studiesamenvatting", size=Pt(14),
                   color=RGBColor(0x55, 0x55, 0x55))
    doc.add_paragraph("")
    note = doc.add_paragraph()
    note.alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_styled_run(note, "Gebaseerd op het handboek Orthopedagogie", italic=True,
